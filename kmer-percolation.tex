...
assemblies. 
% @CTB Two separate arguments: we need big memory NOW.  And, we need to
% scale in the future because sequencing companies are improving.
...
% @CTB phrasing: instead of significantly reduces, maybe ``addresses''?
% Tough statement.  Reduces memory requirements for some operations?
...
\marginpar{@CTB is it true that this was feasible?}
Using the 
...
\marginpar{@CTB Do we want to jump straight to metag?  True for mRNAseq, too;
see my grant text.}

\marginpar{@CTB Driven by the need to scale ... see grant text :).}
...
\marginpar{@CTB drastically conserves is a bit ugly.  Dramatically reduces?
Do we have this comparison down?}
...
\marginpar{@CTB Show that our approach performs well?  We should reword to
use ``compressible graph representation'' in the title and throughout the
text, maybe, too.}
...
% @CTB implementation details of ht (not language, but data struct.)
...
\marginpar{@CTB would suggest P\_f.  Also, eliminate \^ and just make occ(h)
be on the main text line.}
\begin{center}
...
\marginpar{@CTB To calculate the rate at which aberrant connectivity is
generated... avoid use of percolation until results.}
...
\newline
\newline
\marginpar{make equations look prettier and more readable!}
\begin{center}
...
table.
\marginpar{@CTB Ugly text! Make it sound better! (I don't know how...)}
...
\marginpar{@CTB I don't like 'computational demonstrations' unless we simply
cannot calculate it analytically.  Perhaps ``it is straightforward to show...''
and then dump it in supplemental material, along with a computational
demonstration that shows that our code does the same thing.}
...
\marginpar{@CTB This will need to be expanded and moved to the introduction, note;
that's where previous results/citations go.}

\begin{figure}
\center{\includegraphics[width=5in]{figures/optht}}
...
\marginpar{@CTB Steal ``awesome things about this approach'' from grant text,
and discussion section.  Constant memory, works for any k, fast, etc.}
...
\marginpar{@CTB This is a good engineering argument, but a terrible
scientific one!  Refocus on introduction of false connectivity rather
than on practicality of traversal.  Also, should note that it is INTUITIVELY
obvious what the false positive rate for horrible false connectivity
is (roughly 1/8th).}
...
% @CTB ``sufficiently high'' rather than ``above the perc threshold''.
% percolation first mentioned in next sentence.
...
approached.
\marginpar{@CTB Expand discussion of graph partitioning, maybe?  Introduce better? Or do we even want it?}
...
% @CTB wrong, fix :).  Also, ``there is unlikely to be'' sounds like a German
% wrote it.  Fix to English :)

\begin{figure}
\center{\includegraphics[width=5in]{figures/diameter.png}}
...
% @CTB ``aim to show'', blech.  ``We wanted to ascertain at what...''?
...
% @CTB you can use \times  here, too.  I would suggest 4.57e6 notation?
...
% @CTB what's the difference between repeats and identical k-mers, again?
% maybe just ``repeated k-mers''?
...
% @CTB I feel like the German is encroaching here, as well.  ``we find a ... already''
...
% @CTB what does ``is low'' mean?  look at Jellyfish or tallymer?
...
in computation (@CTB).
% @CTB well, what do you think I meant here?  Expand or come ask.
...
% @CTB actually, they DO contribute to the local graph structure.  Why 
% would I say this!? hmm.  Come ask.
...
% @CTB this last sentence is bad.  Discuss effect of FP on this approach.
