...
extremely memory efficient, enabling effective storage of de Bruijn graphs with
4 bits per k-mer.  While the graph contains false nodes and
...
below a specific false positive rate.  We demonstrate the
...
can be handled on commodity computing hardware (@@ science views).  This is especially
...
sequence analysis in metagenomics and transcriptomics (@@ career refs).
...
graph (@@ sutton; pevzner).  In a de Bruijn graph approach, sequencing reads are decomposed
...
similarly large memory requirements (@@ Trinity).
...
up into disconnected components\cite{trinity, metavelvet} (@@ metaIDB). 
...
quadratic in form (@@ citation).  Using the calculation method described
...
One important consideration for the usefulness of our
...
more memory intensive than ours. @@
