...
with as little as 512 GB of system memory \cite{pmid21187386}.  For more
...
Bloom filter, a probabilistic data structure for storing sparse data
...
Thus each k-mer has up to 8 edges to 8 neighbors, which can be determined by
...
In contrast to an exact graph storage, there is a chance that a k-mer
...
in size as the false positive rate increase.
We then
...
begin to join together as a single giant component, and small components
are more rare.
...
percolation theory (@@cite Stauffer). We can map our problem to site percolation by
...
%% @@do we need to demonstrate k independence anywhere? => S1
...
positive rate of 1\%.  While this is not a perfectly fair comparison -- assemblers do more than simply store the graph! -- it does directly suggest that there are opportunities for improving memory usage.
...
under 1.5 GB to partition and assemble -- a decrease in over 20x
...
rate can be tuned based on Bloom filter occupancy, yielding a wide range
of possible storage efficiencies.

Even for low false positive rates such as 5\%, this is still an
efficient graph representation, with a greater than 20-fold
improvement in memory usage over two existing assemblers, Velvet and
ABySS.  We can store k-mers in this data structure with a much smaller
...
the original sequence, unlike sequencing errors which usually have a
low edit distance to the real sequence.  This feature may further
...
\caption{The diameter of randomly generated 58bp long circular
  chromosomes in 8-mer space for different de Bruijn graph false
  positive rates. Only real (non-error) k-mers are considered for
