...
\marginpar{Start with a declarative.  ``We can efficiently store k-mers in a BF.''  That may actually belong in Discussion but either way these paragraphs need a stronger opening!}
...
nodes visited or waypoints in the graph consume addition memory.\marginpar{This last point seems a bit abrupt.  Should we expand?}
...
\marginpar{ARGH.  What does ``impossible'' mean?  I can do it, just not in the time of the universe.  That's ``inconvenient'' not ``impossible.''  Isn't the real point that the graph is not {\em useful} at this point?  Why is that?  It's not just because it's slow: it's because the FP rate results in an overwhelming number of false k-mers!}
...
\marginpar{``aimed'' doesn't really work here.  Re-word into a stronger
opening sentence!}
...
approached.
\marginpar{argh, stop using ``possible'' :).  Reword.}
...
\marginpar{infeasible is better.  but we're not really worried about actually
traversing the graph; we're more worried about the graph just being WRONG WRONG WRONG.}
...
~109\% for $10^7$ bits).
\marginpar{``corresponds to an error rate'' -- could you define error rate, here?  Also, I think we should represent node error raates as 'fp rate (bits/k-mer)' or some such.}
...
\marginpar{...and this means...what? what implications?}
